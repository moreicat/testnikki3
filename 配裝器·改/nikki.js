...
function searchResult() {
    ...
    if (searchTxt) {
        var outSet = [];
        for (var i in clothes) {
            if (clothes[i].isSuit.indexOf(searchTxt) >= 0) {
                outSet.push(clothes[i].isSuit);
            }
        }
        if (outSet.length > 0) {
            ...
            for (var i in outSet) {
                $('#searchResultList').append(button_search(outSet[i], '', 'searchResultSet'));
            }
            $(".searchResultSet").click(function () {
                ...
                for (var i in clothes) {
                    if (clothes[i].isSuit == setName) {
                        $('#searchResultList').append(clothesNameTd_Search(clothes[i]));
                    }
                }
            });
        }
        ...
        for (var b in FEATURES) {
            ...
            for (var i in FEATURES) {
                ...
                if (f == FEATURES[b]) {
                    ...
                    criteria.highscore1 = f;
                }
                if (f == FEATURES[a]) {
                    weight = accMul(weight, 1.778);
                    criteria.highscore2 = f;
                }
                var checked = $('input[name=' + f + ']:radio:checked');
                if (checked.length) {
                    criteria[f] = parseInt(checked.val()) * weight;
...
function filterClotherHTML(btn) {
    ...
    switch ($(btn).text()) {
        ...
        case "活动":
            ...
            break;
        case "迷之屋限定":
            str = "迷,幻,迷/幻,云禅,昼夜,缥缈,昼夜/兑·时光,云禅/兑·卧云,缥缈/兑·翡翠";
            type = -1;
            break;
        case "迷之屋限定染/进":
            str = "迷,幻,迷/幻,云禅,昼夜,缥缈,昼夜/兑·时光,云禅/兑·卧云,缥缈/兑·翡翠";
